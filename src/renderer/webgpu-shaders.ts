...
// Includes transparent variants for WBOIT and an OIT composite shader.
...
export const LAMBERT_TRANSPARENT_WGSL = /* wgsl */ `
struct FrameUniforms {
  viewProjection: mat4x4<f32>,
  lightDir: vec3<f32>,
  lightIntensity: f32,
...
fn fs_main(in: VertexOutput) -> OITOutput {
  ...
  let litColor = baseColor * (ambient + diffuse);
  let color = litColor + emissive;

  // WBOIT weight function
  let z = in.position.z;
  let w = alpha * clamp(0.03 / (1e-5 + pow(z / 200.0, 4.0)), 1e-2, 3e3);
...
export const LAMBERT_SKINNED_TRANSPARENT_WGSL = /* wgsl */ `
struct FrameUniforms {
  viewProjection: mat4x4<f32>,
  lightDir: vec3<f32>,
  lightIntensity: f32,
...
export const BASIC_TRANSPARENT_WGSL = /* wgsl */ `
struct FrameUniforms {
  viewProjection: mat4x4<f32>,
  lightDir: vec3<f32>,
  lightIntensity: f32,
...
export const BASIC_SKINNED_TRANSPARENT_WGSL = /* wgsl */ `
struct FrameUniforms {
  viewProjection: mat4x4<f32>,
  lightDir: vec3<f32>,
  lightIntensity: f32,
...
@group(0) @binding(3) var opaqueTexture: texture_2d<f32>;
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
  let accum = textureSample(accumTexture, texSampler, in.uv);
  let reveal = textureSample(revealTexture, texSampler, in.uv).r;
  let opaqueColor = textureSample(opaqueTexture, texSampler, in.uv).rgb;

  // No transparent fragments — keep opaque color
  if (accum.a < 1e-5) {
    return vec4<f32>(opaqueColor, 1.0);
  }

  let avgColor = accum.rgb / max(accum.a, 1e-5);

  // Blend transparent over opaque
  let result = avgColor * (1.0 - reveal) + opaqueColor * reveal;
