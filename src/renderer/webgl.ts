...
// MSAA MRT draw (opaque + transparent) → MSAA resolve → bloom post-processing →
// final blit with gamma correction.
//
// Transparency uses sorted back-to-front alpha blending drawn in the SAME MSAA render pass
// as opaque meshes (before the MSAA resolve blit). Transparent meshes use the same shader
// programs as opaque meshes but with different GL state: alpha blending enabled, depth writes
// disabled, and back-face culling disabled. This avoids the need for separate transparency
// textures, framebuffers, or extensions.
//
...
export class WebGLRenderer implements Renderer {
  ...
  render(scene: Scene, camera: PerspectiveCamera) {
    ...
    // ─── Transparent meshes (same MSAA pass, alpha blend) ─────────
    if (transparentStart < meshes.length) {
      // Change GL state for transparency
