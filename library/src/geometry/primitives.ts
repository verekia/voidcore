...
//
// Each class extends Geometry and builds vertex data (positions, normals, UVs, indices) from
// scratch using math. All shapes are Z-up and centered at the origin. The process is always
// the same: loop over a grid of parameters (angles, segments), compute each vertex's 3D
// position and surface normal, generate UV coordinates for texturing, then connect vertices
// into triangles via an index array.
//
// new PlaneGeometry()    – Flat rectangular surface on the XY plane.
// new BoxGeometry()      – Six-faced box (cube/cuboid).
// new SphereGeometry()   – UV sphere with poles along the Z axis.
// new ConeGeometry()     – Cone with a circular base and a pointed tip.
// new CylinderGeometry() – Cylinder with configurable top/bottom radii.
...
export class ConeGeometry extends Geometry {
  constructor(opts: ConeGeometryOptions = {}) {
...
export class CapsuleGeometry extends Geometry {
  constructor(opts: CapsuleGeometryOptions = {}) {
...
export class CircleGeometry extends Geometry {
  constructor(opts: CircleGeometryOptions = {}) {
    const r = opts.radius ?? 1
